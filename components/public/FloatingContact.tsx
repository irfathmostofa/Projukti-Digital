...
export function FloatingContact({
  ...
  const buttons = [
    {
      href: s.whatsapp
        ? `https://wa.me/${s.whatsapp.replace(/[^0-9]/g, "")}`
        : null,
      icon: MessageCircle,
    ...
    <div className="fixed bottom-5 right-5 z-40 flex flex-col items-end gap-3">
      {buttons.map(({ href, icon: Icon, label, color }, i) => (
        ...
        >
          ...
          <span
            className={`relative flex h-11 w-11 items-center justify-center rounded-full text-white shadow-lg ${color}`}
          >
            <span
              className="absolute inset-0 rounded-full opacity-60 animate-ping"
              style={{ backgroundColor: "inherit", animationDuration: "3s" }}
              aria-hidden="true"
            />
